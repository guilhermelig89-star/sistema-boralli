...
export default function RepararPacotesPage() {
  ...
  async function simularLimpezaJoana() { setLoading(true); try { setResumo(null); setSimulacao({ tipo: "joana", ...(await limparConsumosJoana({ simulacao: true })) }); } finally { setLoading(false); } }
  async function aplicarLimpezaJoana() { if (!confirm("Aplicar limpeza dos consumos da Joana? Essa ação remove vínculos de consumo e recalcula os saldos.")) return; setLoading(true); try { const r = await limparConsumosJoana({ simulacao: false }); setResumo({ tipo: "joana", ...r }); setSimulacao(null); await carregar(); } finally { setLoading(false); } }

  return <main className="reparo-page"><h1>Administração &gt; Reparar pacotes</h1>
    <div className="reparo-acoes"><button onClick={carregar} disabled={loading}>Listar inconsistências</button><button onClick={simularRecalculo} disabled={loading}>Simular recálculo global</button>{simulacao?.tipo === "global" && <button onClick={aplicarRecalculo} disabled={loading}>Aplicar correção global</button>}<button onClick={simularLimpezaJoana} disabled={loading}>Preview limpeza da Joana</button><button onClick={aplicarLimpezaJoana} disabled={loading}>Limpar consumos da Joana</button></div>
    {simulacao && <section className="reparo-box"><h2>Modo simulação</h2><pre>{JSON.stringify(simulacao, null, 2)}</pre>{!simulacao.tipo && <button onClick={() => aplicarCorrecao(simulacao.pacoteId)} disabled={loading}>Aplicar correção</button>}{simulacao.tipo === "joana" && <button onClick={aplicarLimpezaJoana} disabled={loading}>Executar limpeza da Joana</button>}</section>}
