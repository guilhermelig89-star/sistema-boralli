...
export default function RepararPacotesPage() {
  ...
  async function aplicarCorrecao(pacoteId = simulacao?.pacoteId) { if (!pacoteId) return; if (!confirm("Aplicar correção do pacote?")) return; setLoading(true); try { const r = await corrigirPacoteInconsistente({ pacoteId, simulacao: false }); setResumo(r); setSimulacao(null); await carregar(); } finally { setLoading(false); } }
  ...
  return <main className="reparo-page"><h1>Administração &gt; Reparar pacotes</h1>
    ...
    {resumo && <section className="reparo-box"><h2>Resumo da correção</h2><pre>{JSON.stringify(resumo, null, 2)}</pre></section>}
